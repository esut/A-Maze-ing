...
def find_shortest_path(
        maze: List[List[int]],
        entry: Tuple[int, int],
        exit_: Tuple[int, int]) -> Optional[List[Tuple[int, int]]]:
...
def embed_42_pattern(
    ...
    for x in range(entry_x):
        if ((x, entry_y) not in blocked
                and (x + 1, entry_y) not in blocked):
            maze[entry_y][x] &= ~EAST
            maze[entry_y][x + 1] &= ~WEST

    for x in range(exit_x):
        if ((x, exit_y) not in blocked
                and (x + 1, exit_y) not in blocked):
            maze[exit_y][x] &= ~EAST
...
class MazeGenerator:
    ...
    def generate(
        ...
        self.maze = generate_maze(
            self.width, self.height, entry, exit_, self.seed
        )
    ...
    def get_solution(self) -> Optional[List[Tuple[int, int]]]:
        ...
        Returns:
            List of (x, y) cells forming the path, or None if no maze
        """
